...
class DashboardController:
    ...
    def get_metrics():
        ...
        # Check if business_id is provided as query parameter
        ...
        print(
            f"DEBUG: Dashboard metrics for User ID: {g.current_user.id}, Business ID: {business_id}"
        )
        ...
        print(
            f"DEBUG: Found {len(current_period_transactions)} transactions in period."
        )
        ...
        current_inflow = sum(
            t.amount for t in current_period_transactions if t.direction == "inflow"
        )
        current_outflow = sum(
            t.amount for t in current_period_transactions if t.direction == "outflow"
        )
        net_cashflow = float(current_inflow - current_outflow)

        prev_inflow = sum(
            t.amount for t in prev_period_transactions if t.direction == "inflow"
        )
        prev_outflow = sum(
            t.amount for t in prev_period_transactions if t.direction == "outflow"
        )
        ...
        if prev_net_cashflow != 0:
            net_cashflow_percentage_change = (
                (net_cashflow - prev_net_cashflow) / abs(prev_net_cashflow)
            ) * 100
        ...
        current_cash = (
            business.settings.get("current_cash", 0) if business.settings else 0
        )
        average_monthly_expenses = (
            current_outflow  # Using last 30 days outflow as proxy
        )
        ...
        if current_inflow + current_outflow > 0:
            cashflow_volatility = (
                abs(current_inflow - current_outflow)
                / (current_inflow + current_outflow)
            ) * 100
        cashflow_volatility_trend = (
            "neutral"  # Needs historical volatility to calculate trend
        )
        ...
        latest_risk_score = (
            ...
            .order_by(RiskScore.assessed_at.desc())
            .first()
        )
        projected_risk = (
            latest_risk_score.details.get("overall_risk", "Unknown")
            if latest_risk_score and latest_risk_score.details
            else "Medium"
        )
        projected_risk_status = (
            latest_risk_score.details.get("recommendations", [])
            if latest_risk_score and latest_risk_score.details
            else ["Monitor"]
        )
        ...
        if latest_risk_score and latest_risk_score.details:
            ...
            risk_breakdown.append(
                {
                    ...
                    "level": float(latest_risk_score.cashflow_risk_score),
                    "description": "Based on current net cashflow trends.",
                }
            )
            risk_breakdown.append(
                {
                    "category": "Liquidity Risk",
                    "level": float(latest_risk_score.liquidity_score),
                    "description": "Based on current cash vs expenses.",
                }
            )
            risk_breakdown.append(
                {
                    "category": "Volatility Index",
                    "level": float(latest_risk_score.volatility_index * 100),
                    "description": "Measures fluctuation of cashflow.",
                }
            )
        else:
            ...
            risk_breakdown = [
                ...
                },
                {
                    "category": "Liquidity Risk",
                    "level": 60,
                    "description": "Default liquidity assessment.",
                },
                {
                    "category": "Burn Rate Risk",
                    "level": 40,
                    "description": "Default burn rate assessment.",
                },
            ]
        ...
        return jsonify(
            {
                ...
                "net_cashflow_percentage_change": round(
                    net_cashflow_percentage_change, 2
                ),
                "liquidity_score": round(liquidity_score, 2),
                "liquidity_score_trend": liquidity_score_trend,
                "liquidity_score_percentage_change": round(
                    liquidity_score_percentage_change, 2
                ),  # Placeholder
                "cashflow_volatility": round(cashflow_volatility, 2),
                "cashflow_volatility_trend": cashflow_volatility_trend,  # Placeholder
                "cashflow_volatility_percentage_change": round(
                    cashflow_volatility_percentage_change, 2
                ),  # Placeholder
